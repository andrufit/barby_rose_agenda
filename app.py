...
def agendar():
    ...
    if request.method == "POST":
        ...
        # 🔹 Insert principal
        cursor.execute("""
            INSERT INTO citas (nombre, telefono, servicio, fecha, horario, empleada, duracion)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """, (cliente, telefono, nombre_servicio, fecha, horario, empleada, duracion))
...
def actualizar_cita():
    ...
    cursor.execute(
        """
        UPDATE citas
        SET nombre=?, telefono=?, servicio=?, fecha=?, horario=?, empleada=?, duracion=?
        WHERE id=?
        """,
        (cliente, telefono, servicio, fecha, hora, empleada, duracion, cita_id_int),
    )
...
def recordatorios_whatsapp():
    ...
    cursor.execute(
        """
        SELECT c.id, c.nombre, c.servicio, c.fecha, c.horario, c.empleada, c.telefono
        FROM citas c
        WHERE c.fecha=?
        ORDER BY c.horario
        """,
